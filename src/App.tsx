...
function mockRevealPass(idType: "email" | "phone", value: string) {
  return idType === "email"
    ? /owner|ana|michael/.test(value.toLowerCase())
    : /212$/.test(value);
}
...
(function devSmokeTests() {
  try {
    console.assert(
      Array.isArray(DEMO_BUSINESSES) && DEMO_BUSINESSES.length > 0,
      "DEMO_BUSINESSES should have items"
    );
    console.assert(
      Object.keys(brand).includes("orange") &&
        Object.keys(brand).includes("bg"),
      "brand tokens present"
    );
    // Ensure search concatenation logic works without template literals
    const sampleQ = "grand";
    const hits = DEMO_BUSINESSES.filter((b) =>
      (b.name + " " + b.addr).toLowerCase().includes(sampleQ)
    );
    console.assert(
      hits.length >= 1,
      "string concatenation search should find at least one business"
    );
    // Check reveal helper
    console.assert(mockRevealPass("email", "owner@example.com") === true, "email reveal should pass");
    console.assert(mockRevealPass("phone", "5551212") === true, "phone reveal should pass when ends with 212");
  } catch {}
})();
  ...
  const results = useMemo(() => {
    ...
    if (view === "search-business") {
      return DEMO_BUSINESSES.filter((b) =>
        (b.name + " " + b.addr).toLowerCase().includes(q)
      );
    }
    if (view === "search-property") {
      return DEMO_PROPERTIES.filter((p) =>
        (p.address + " " + p.bbl).toLowerCase().includes(q)
      );
    }
    return [] as any[];
  }, [query, view]);
  ...
  return (
    <div className="min-h-screen" style={{ background: brand.bg }}>
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 grid gap-6">
          ...
          <>
            ...
            >
              <div className="grid md:grid-cols-3 gap-3">
                ...
                <button
                  ...
                  onClick={() =>
                    alert(
                      "(Demo) Session created. Now set the Portal Access Password below."
                    )
                  }
                >
                  Save session
            ...
            >
              ...
              <p className="text-xs text-gray-500 mt-2">
                In production this calls <code>/.netlify/functions/portal.password</code>.
              </p>
            </Section>
            ...
            <Section title="How it works">
              <ul className="text-sm text-gray-700 list-disc pl-5 space-y-2">
                <li>
                  Public search returns non-PII only (record basics + associated
                  contact <b>names</b>).
                </li>
                <li>
                  A contact can <b>reveal</b> full details by entering a
                  matching email or phone (server-side check).
                </li>
                <li>
                  Portal access is protected by a tenant password that you set
                  above.
                </li>
              </ul>
            </Section>
        ...
        {view === "access" && (
          <>
            ...
            >
              <div className="grid md:grid-cols-3 gap-3">
                ...
                <button
                  ...
                  style={{ background: brand.orange }}
                  onClick={() => {
                    if (!accessCompanyId || !accessPw)
                      return alert("Enter Community ID and password");
              ...
              <p className="text-xs text-gray-500 mt-2">
                ...
                <code>/.netlify/functions/access.login</code> and sets an
                HttpOnly cookie.
              </p>
            </Section>
            ...
            <Section
              ...
              subtitle="If your API key was rotated or portal password changed, update it here without redeploying."
            >
              <div className="grid md:grid-cols-3 gap-3">
                <input
                  className="border rounded-lg p-2"
                ...
                <button
                  ...
                  style={{ background: brand.teal }}
                  onClick={() => {
                    alert("(Demo) API key updated for this session.");
                  }}
                >
                  Save new API Key
                </button>
              ...
              <p className="text-xs text-gray-500 mt-2">
                This would call <code>/.netlify/functions/portal.credentials</code> to
                persist encrypted per tenant.
              </p>
            </Section>
          </>
        )}
                  ...
                  <div key={b.id} className="rounded-2xl border p-4 bg-white">
                    ...
                    <div className="mt-3 text-sm">
                      <div className="text-gray-600">
                        Associated Contacts (names only)
                      </div>
        ...
        {view === "add-business" && (
          <AddBusinessView onCancel={() => setView("search-business")} />
        )}
        ...
        {view === "details" && detail && (
          <DetailsView
            detail={detail}
            goBack={() =>
              setView(detail.type === "business" ? "search-business" : "search-property")
            }
          />
        )}
      </main>
    ...
    >
      <div className="p-8 md:p-12 text-white">
        <h1 className="text-2xl md:text-3xl font-semibold">
          Stakeholder Self-Service Lookup
        </h1>
        <p className="mt-2 text-white/80 max-w-2xl">
          Let property owners and business representatives look up their record,
          verify identity, reveal contact details, submit updates, or add a
          missing business—without creating an account.
        </p>
      ...
      <div
        className="absolute -right-10 -bottom-10 w-64 h-64 rounded-full opacity-20"
        style={{ background: brand.teal }}
      />
    </div>
  );
...
function DetailsView({
  ...
  function handleReveal() {
    // Mock: if email contains "owner" or phone ends with 212, reveal contacts
    const pass =
      idType === "email"
        ? /owner|ana|michael/.test(idValue.toLowerCase())
        : /212$/.test(idValue);
    ...
    setRevealMsg(
      pass ? "Verified – details revealed." : "No match. Try a different email or phone."
    );
  ...
  return (
    <div className="grid gap-6">
      ...
      <Section
        title={
          detail.type === "business"
            ? (mock as any)?.name || "Business"
            : (mock as any)?.address || "Property"
        }
        subtitle={
          detail.type === "business"
            ? (mock as any)?.addr
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Section
          ...
          subtitle="Add a missing contact or suggest corrections."
        >
          <form
            ...
            onSubmit={(e) => {
              e.preventDefault();
              alert(
                "Thanks! In the live version this posts to /.netlify/functions/contact.submit"
              );
              (e.target as HTMLFormElement).reset();
              console.log("EVENT:", "contact_update_submitted", {
                record_type: detail.type,
                record_id: detail.id,
              });
            }}
          >
            ...
            <button
              className="rounded-xl px-4 py-2 text-white font-medium mt-1 w-fit"
              style={{ background: brand.teal }}
            >
              Submit update
            </button>
      ...
      >
        ...
        >
          ...
          <input
            className="border rounded-lg p-2"
            name="category"
            placeholder="Category (e.g., Retail > Grocery)"
          />
          <input
            className="border rounded-lg p-2"
            name="tags"
          ...
          <input
            className="border rounded-lg p-2"
            name="property_municipal_id"
            placeholder="Property municipal ID (optional)"
          />
          <input
            className="border rounded-lg p-2"
            name="unit_name"
          ...
          <div className="md:col-span-2 mt-4 font-medium text-gray-700">
            Primary contact (admin fields)
          </div>
          <input
            className="border rounded-lg p-2"
        ...
        <p className="text-xs text-gray-500 mt-3">
          Field mapping applied: <b>url</b> for website; <b>primary_contact_*</b> and{" "}
          <b>secondary_contact_*</b> for admin contacts; <b>email/phone</b> reserved
          for public business contact info.
        </p>
      </Section>
    </div>
  );
